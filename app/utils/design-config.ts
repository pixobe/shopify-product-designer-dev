...
const variantComparisonKey = (value: string | number | null | undefined): string | null => {
...
const fetchSettingsMetaobject = async (admin: any) => {
  ...
  if (Array.isArray(body.errors) && body.errors.length > 0) {
    console.error("Unable to load Pixobe design settings metaobject", body.errors);
  }
...
export const loadPixobeDesignSettings = async (admin: any) =>
  (await fetchSettingsMetaobject(admin)) ?? {};

export const loadPixobeProductMedia = async (
  admin: any,
  ...
  const entries = await fetchProductMetafield(admin, productId);
